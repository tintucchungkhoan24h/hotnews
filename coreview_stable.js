...
function updateStableViewLabels() {
    ...
    const fastNewsBtnTextStable = document.getElementById('fastNewsBtnTextStable');
    if (fastNewsBtnTextStable) {
        fastNewsBtnTextStable.innerText = window._fastNewsMode ? 
            (i18n[stateStable.lang].hideFastNewsBtn || '❌ Ẩn tóm tắt') : 
            (i18n[stateStable.lang].fastNewsBtn || '⚡ Điểm tin nhanh');
    }
}

window.toggleFastNewsStable = function() {
    if (window.toggleFastNewsGeneric) {
        window.toggleFastNewsGeneric('fastNewsBtnTextStable', renderStableBody);
    }
};
...
function setupStableViewEventListeners() {
    ...
    if (firstBtn) {
        firstBtn.onclick = () => { 
            stateStable.currentPage = 0; 
            const _anchor = document.getElementById('prevBtnStable');
            const _rect = _anchor ? _anchor.getBoundingClientRect() : null;
            fetchStableData().then(() => handlePaginationScrollStable(_rect)); 
        };
    }
...
function renderStableBody() {
    ...
    tbody.innerHTML = stateStable.data.map((row, idx) => {
        ...
        return `
            <tr class="hover:bg-fin-gold/5 transition-colors"
                ...
                <td class="px-2 py-2 text-center text-gray-500 text-xs">${(stateStable.currentPage * stateStable.pageSize) + idx + 1}</td>
                <td class="px-2 py-2 text-gray-400 text-xs">${formatStableDateTime(row.publish_time)}</td>
                <td class="px-2 py-2">
    ...
    if (window._fastNewsMode && window.injectFastNewsRowsGeneric) {
        window.injectFastNewsRowsGeneric('tableBodyStable', 'stable');
    }
}
...
function handlePaginationScrollStable(rectBefore) {
    ...
    if (isShortPage) {
        ...
        const anchor = document.getElementById('prevBtnStable');
        if (anchor) {
