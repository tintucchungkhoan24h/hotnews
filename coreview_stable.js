...
function renderStableBody() {
    ...
    tbody.innerHTML = stateStable.data.map((row, idx) => {
        ...
        const ma15Tooltip = ma15Status.key === 'N/A'
            ? i18n[stateStable.lang].ma15Tooltip.na
            : (i18n[stateStable.lang].ma15Tooltip[ma15Status.key] || '');
        ...
        return `
            <tr class="hover:bg-fin-gold/5 transition-colors">
                ...
                <td class="px-4 py-4">
                    <span class="stock-badge" onclick="openTvChart('${row.single_stock || ''}')" title="${i18n[stateStable.lang].chartTooltip}">
                        ${row.single_stock || '-'}
