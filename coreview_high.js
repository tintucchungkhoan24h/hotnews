...
function renderHighHeaders() {
    ...
    document.getElementById('tableHeaderHigh').innerHTML = cols.map(c => `
        <th class="px-2 py-4 ${c.s ? 'cursor-pointer hover:bg-gray-700/50' : ''} ${c.w} ${c.s ? getHighSortClass(c.id) : ''}" 
    ...
    tbody.innerHTML = stateHigh.data.map((row, idx) => {
        ...
        return `
            <tr class="hover:bg-fin-gold/5 transition-colors"
                ...
                <td class="px-2 py-2 text-center text-gray-500 text-xs">${(stateHigh.currentPage * stateHigh.pageSize) + idx + 1}</td>
                <td class="px-2 py-2 text-gray-400 text-xs">${formatHighDateTime(row.publish_time)}</td>
                <td class="px-2 py-2">
