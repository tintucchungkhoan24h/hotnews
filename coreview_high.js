...
function renderHighBody() {
    ...
    tbody.innerHTML = stateHigh.data.map((row, idx) => {
        ...
        const ma15Tooltip = ma15Status.key === 'N/A'
            ? i18n[stateHigh.lang].ma15Tooltip.na
            : (i18n[stateHigh.lang].ma15Tooltip[ma15Status.key] || '');
...
async function exportHighToExcel() {
    ...
    try {
        ...
        XLSX.utils.book_append_sheet(wb, ws, i18n[stateHigh.lang].excelSheetName);

        // Generate filename with date range
        const filename = `TinTucChungKhoan_${stateHigh.fromDate}_${stateHigh.toDate}.xlsx`;
