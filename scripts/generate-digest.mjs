...
async function fetchSummaries() {
  ...
  let url = `${SUPABASE_URL}/rest/v1/market_summary_stock?order=summary_date.desc,created_at.desc&limit=30&select=summary_date,created_at,articles`;
...
function generatePage({ articlesList, lang, headerHtml, footerHtml, langs, clientCss, clientJs, archiveItems }) {
  const canonical  = `${SITE_BASE}/diem-tin-chung-khoan/${lang}/`;
  ...
  const hubTitle = (currentI18n.video?.titleStock || currentI18n.tabs?.digest || 'Stock Market Digest') + ' | ' + SITE_NAME;
