...
function buildSpotlightFilterQuery() {
    ...
    if (typeof filterState !== 'undefined' && filterState.spotlight) {
        ...
        if (filterState.spotlight.priceChange && typeof filterState.spotlight.priceChange.min === 'number' && typeof filterState.spotlight.priceChange.max === 'number') {
            const minPriceChange = filterState.spotlight.priceChange.min;
            const maxPriceChange = filterState.spotlight.priceChange.max;
        ...
        if (stateSpotlight.sortCol === 'headline') {
            ...
            if (!Array.isArray(json)) {
                ...
                if (stateSpotlight.sortCol === 'price_change_today_pct') {
                    ...
                    console.debug('[Spotlight] Sorting %Chg client-side, direction:', stateSpotlight.sortDesc ? 'DESC' : 'ASC');
...
function renderSpotlightHeaders() {
    ...
    document.getElementById('tableHeaderSpotlight').innerHTML = cols.map(c => `
        <th class="px-2 py-4 ${c.s ? 'cursor-pointer hover:bg-gray-700/50' : ''} ${c.w} ${c.s ? getSortClassSpotlight(c.id) : ''}" 
...
function handleSpotlightSort(id) {
    ...
    if (id === 'price_change_today_pct') {
        console.debug('[Spotlight] handleSpotlightSort: %Chg clicked. sortCol=', stateSpotlight.sortCol, 'sortDesc=', stateSpotlight.sortDesc ? 'DESC' : 'ASC');
    }
    fetchSpotlightData();
}
    ...
    tbody.innerHTML = stateSpotlight.data.map((row, idx) => {
        ...
        return `
            <tr class="hover:bg-fin-gold/5 transition-colors"
                ...
                <td class="px-2 py-2 text-center text-gray-500 text-xs">${(stateSpotlight.currentPage * stateSpotlight.pageSize) + idx + 1}</td>
                <td class="px-2 py-2 text-gray-400 text-xs">${formatDateTimeSpotlight(row.publish_time)}</td>
                <td class="px-2 py-2">
                    <span class="stock-badge" onclick="openTvChart('${row.single_stock || ''}', '${row.com_group_code || ''}')" title="${i18n[lang].chartTooltip}">
                        ${row.single_stock || '-'}
...
window.exportSpotlightToExcel = async function exportSpotlightToExcel(btnRef) {
    ...
    try {
        ...
        if (stateSpotlight.sortCol === 'price_change_today_pct') {
            const dir = stateSpotlight.sortDesc ? -1 : 1;
            console.debug('[Spotlight][Export] Sorting %Chg client-side, direction:', stateSpotlight.sortDesc ? 'DESC' : 'ASC');
            allData.sort((a, b) => {
                const av = Number(a.price_change_today_pct ?? 0);
                const bv = Number(b.price_change_today_pct ?? 0);
                return dir * (av - bv);
