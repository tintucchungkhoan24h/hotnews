...
function updateWatchlistViewLabels() {
    ...
    const fastNewsBtnTextWatchlist = document.getElementById('fastNewsBtnTextWatchlist');
    if (fastNewsBtnTextWatchlist) {
        fastNewsBtnTextWatchlist.innerText = window._fastNewsMode ? 
            (i18n[lang].hideFastNewsBtn || '❌ Ẩn tóm tắt') : 
            (i18n[lang].fastNewsBtn || '⚡ Điểm tin nhanh');
    }
}

window.toggleFastNewsWatchlist = function() {
    if (window.toggleFastNewsGeneric) {
        window.toggleFastNewsGeneric('fastNewsBtnTextWatchlist', renderWatchlistBody);
    }
};
...
function refreshWatchlistData() {
    ...
    const _wLang = (typeof state !== 'undefined' ? state.lang : null) || stateWatchlist.lang;
    if (window.headlineTranslationCache && _wLang && _wLang !== 'vi') {
        const suffix = `_${_wLang}`;
        Object.keys(window.headlineTranslationCache).forEach(key => {
            if (key.endsWith(suffix)) {
                delete window.headlineTranslationCache[key];
            }
        });
        console.log(`[Refresh Watchlist] Cleared translation cache for lang="${_wLang}"`);
    }
...
function renderWatchlistBody() {
    ...
    tbody.innerHTML = stateWatchlist.data.map((row, idx) => {
        ...
        return `
            <tr class="hover:bg-fin-gold/5 transition-colors"
                ...
                data-time="${formatDateTimeWatchlist(row.publish_time)}"
                onclick="window.toggleNewsQuoteTooltip&&toggleNewsQuoteTooltip(event, this)">
                <td class="px-2 py-2 text-center text-gray-500 text-xs">${(stateWatchlist.currentPage * stateWatchlist.pageSize) + idx + 1}</td>
                <td class="px-2 py-2 text-gray-400 text-xs">${formatDateTimeWatchlist(row.publish_time)}</td>
                <td class="px-2 py-2">
    ...
    bindWatchlistRowTouchEvents();

    if (window._fastNewsMode && window.injectFastNewsRowsGeneric) {
        window.injectFastNewsRowsGeneric('tableBodyWatchlist', 'watchlist');
    }
}

// Touch support: highlight stock badge when user taps any cell in a row
function bindWatchlistRowTouchEvents() {
    const tbody = document.getElementById('tableBodyWatchlist');
